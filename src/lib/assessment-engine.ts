  ...
  },
  {
    ...
    options: [
      ...
      },
      {
        ...
        vector: [0, 0, 0, 1, 2, 1],
        clusterScores: { 'creative': 2, 'analytical': 1 },
      },
      {
        id: 'r2_d',
...
function computeConfidence(clusterScores: Record<ClusterId, number>): {
  ...
  return {
    topCluster: sorted[0][0],
    margin,
    needsTieBreaker: margin < 8,
  };
}
...
export function getNextQuestions(
  ...
  const clusterScores = computeClusterScores(phase1Options);
  const { topCluster, needsTieBreaker } = computeConfidence(clusterScores);
...
export function scoreAssessment(
  ...
  const { topCluster, margin } = computeConfidence(clusterScores);
  ...
  // Step 5: Score all 12 roles via weighted cosine similarity
  const scoredRoles = ROLE_ORDER.map((roleId) => {
    ...
    let score = Math.max(
      ...
      Math.min(
        99,
        Math.round(
          weightedCosineSimilarity(userVector, role.vector, role.dimensionWeights) * 100
        )
      )
    );

    // Apply disqualifier rules
    ...
    const rationaleSignals = allSelected
      .map((option) => ({
        ...
        alignment: signalAlignment(option, role),
      }))
  ...
  // Step 6: Build topRoles — prefer winning cluster, fill to 3
  const clusterRoleIds = CLUSTER_ROLES[topCluster];
  const clusterScored = scoredRoles
    .filter((r) => clusterRoleIds.includes(r.roleId))
    .sort((a, b) => b.score - a.score);
  const otherScored = scoredRoles
    .filter((r) => !clusterRoleIds.includes(r.roleId))
    .sort((a, b) => b.score - a.score);

  const orderedCandidates = [...clusterScored, ...otherScored];
  const positive = orderedCandidates.filter((r) => r.score > 0);
  const candidates = positive.length >= 3 ? positive : orderedCandidates;
